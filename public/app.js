...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function formatMarkdown(text) {
    ...
    // Escape HTML tags to prevent browser rendering raw tags as HTML elements
    ...
    html = html.replace(/```([a-zA-Z0-9:#+-]+)?\n([\s\S]*?)(?:```|$)/g, (match, lang, code) => {
      ...
      if (isPreviewable) {
        return `
          <div class="code-preview-container" id="${blockId}">
            <div class="code-header-tabs">
              ...
              <button class="preview-action-btn" onclick="openRightPreview('${blockId}')">
        ...
        return `<pre><code class="language-${cleanLang || 'txt'}">${cleanCode}</code></pre>`;
      }
    });
    ...
    // Line breaks: replace newlines with <p> tags or <br>
    const paragraphs = html.split('\n\n');
    html = paragraphs.map(p => {
      if (p.trim().startsWith('<div class="code-preview-container"') || (p.trim().startsWith('<pre>') && p.trim().endsWith('</pre>'))) {
        return p;
      }
      return `<p>${p.replace(/\n/g, '<br>')}</p>`;
    }).join('');
